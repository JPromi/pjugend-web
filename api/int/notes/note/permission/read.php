...
if(isset($_REQUEST["id"]) && isset($_REQUEST["user"])) {
    ...
    if(isset($note)) {
        ...
        if(isset($getperm)) {
            if($getperm["permission"] == "write") {
                $con->query("UPDATE notes_permission SET permission = 'read' WHERE note_id = $tmp_note AND user_id = $tmp_user");
            ...
            $con->query("INSERT INTO notes_permission (note_id, user_id, permission) VALUES ($tmp_note, $tmp_user, 'read')");
            echo json_encode(array(
                'status' => 'ok',
                'action' => 'set',
            ));
