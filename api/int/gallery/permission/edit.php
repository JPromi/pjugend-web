...
if(isset($_REQUEST["id"]) && isset($_REQUEST["user"])) {
    ...
    if(isset($gallery)) {
        ...
        if(isset($getperm)) {
            if($getperm["permission"] != "edit") {
                $con_public->query("UPDATE gallery_permission SET permission = 'edit' WHERE gallery_id = $tmp_gallery_id AND user_id = $tmp_user");
            ...
            $con_public->query("INSERT INTO gallery_permission (gallery_id, user_id, permission) VALUES ($tmp_gallery_id, $tmp_user, 'edit')");
            echo json_encode(array(
                'status' => 'ok',
                'action' => 'set',
            ));
